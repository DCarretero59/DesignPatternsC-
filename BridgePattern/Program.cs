...
namespace BridgePattern
{
    ...
    public class Circle : Shape
    {
        ...
        public const string VariableName = nameof(radius);
    ...
    public class Demo
    {
        static void Main(string[] args)
        {
            ...
            cb.Register((c, p) => new Circle(c.Resolve<VectorRenderer>(),
                p.Named<float>(Circle.VariableName)));
            using (var c = cb.Build())
            {
                var circle = c.Resolve<Circle>(
                    new NamedParameter(Circle.VariableName, 5.0f)
                );
                circle.Draw();
                circle.Resize(2);
                circle.Draw();
